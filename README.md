...
- 不提交真实数据库密码、JWT 密钥、AccessKey 或 `.env` 文件。
- 提交到仓库的 `application.yml` 只保留占位符配置；本地真实配置不要提交。
- 前端配置文件默认使用占位符；提交前确认没有不想公开的备案号、邮箱、社交账号或线上接口地址。
- 根目录 `blog.sql` 用于初始化数据库，提交前应确认其中不包含不该公开的个人数据或生产账号。
